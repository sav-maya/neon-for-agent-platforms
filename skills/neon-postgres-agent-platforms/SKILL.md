...
**Partners cloning the repo:** follow **[README, Quick start](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/README.md#quick-start)** (repository root), the skill folder **[doc index](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/README.md)** (`references/README.md`: reading order + symlink table, not a second Quick start), and **[MANAGEMENT_API_SAMPLES.md](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/MANAGEMENT_API_SAMPLES.md)** for scripts and env vars. This file is for **assistants** (after `**neon-postgres`**); it does not replace that skill.

This directory follows the **[Agent Skills](https://agentskills.io/specification)** layout (`SKILL.md`, optional `references/`). Runnable Management API samples live under **`scripts/`** at the **[repository root](https://github.com/neondatabase/neon-for-agent-platforms)** (not inside this skill folder). Symlinked **`*.ts`** copies for assistants live **beside this skill’s reference docs** in **`references/`**.
...
- **Checkpoints on platforms**: A tenant checkpoint is usually a **compound record** (source revision + Neon snapshot/branch + secrets/env version + deploy URL + agent metadata). Do not equate “checkpoint” with “Neon branch” alone, see the **[compound checkpoints doc](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/COMPOUND_CHECKPOINTS_FOR_AGENT_PLATFORMS.md)**.
...
- For **project increase requests** (or related org capacity changes), email **[agents@neon.tech](mailto:agents@neon.tech)**. Ask the partner to include **organization ID(s)** (free + paid if both), brief **growth / usage context**, and any **timeline**, and to continue using **shared Slack** if they already have a channel with Neon.
...
- Persist **snapshot and branch ids per tenant** in your ledger; tie each to **non-Neon state** (**[compound checkpoints](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/COMPOUND_CHECKPOINTS_FOR_AGENT_PLATFORMS.md)**).
...
- Pair branch/snapshot lifecycle with **secrets rotation** and **deploy URLs** via **[compound checkpoints](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/COMPOUND_CHECKPOINTS_FOR_AGENT_PLATFORMS.md)** so previews stay coherent.
...
- **REST vs Postgres vs billing confusion**: run `**auth-users.ts meta`** from `**[scripts/](https://github.com/neondatabase/neon-for-agent-platforms/tree/main/scripts)**` for a short routing map (Neon Auth REST vs Postgres roles vs consumption APIs).
...
- **Limit increases** (project caps, rate limits, etc.): Email **[agents@neon.tech](mailto:agents@neon.tech)** with org ID(s), growth context, and timeline. Also flag in the shared Slack channel.
...
The repository **[README.md](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/README.md)** (**Quick start**) is the human entry point. The skill’s **[references doc index](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/README.md)** (`references/README.md`) lists **reading order** for deeper docs and symlinked **`*.ts`** only. **All** runnable scripts live under **`scripts/`** at the repo root; **`references/`** mirrors each **`*.ts`** as a symlink for skill-local browsing.

- **[Fleet and org model](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/README.md#fleet-and-org-model-summary)**: short two-org summary at the repo README.
- **[MANAGEMENT_API_SAMPLES.md](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/MANAGEMENT_API_SAMPLES.md)**: full script list, env vars, commands.
- **[Compound checkpoints](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/COMPOUND_CHECKPOINTS_FOR_AGENT_PLATFORMS.md)** then **[checkpoint orchestration](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/CHECKPOINT_ORCHESTRATION_PATTERN.md)**: what to store vs how layers fit (see the **`references/`** [doc index](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/README.md) reading order).
- **Neon TypeScript SDK** in these samples means **`@neondatabase/api-client`** only (no other Neon npm packages). Samples use it directly (`createApiClient`); shared **[utils.ts](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/scripts/utils.ts)** (also **[references/utils.ts](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/skills/neon-postgres-agent-platforms/references/utils.ts)**) only polls async operations. Fleet-oriented flows include create/delete project, branches, snapshots + **`versioning-flow.ts`** ([database versioning](https://neon.com/docs/ai/ai-database-versioning)), org transfer, consumption **v2**, Neon Auth admin routes (**`auth-users.ts meta`** distinguishes REST Auth vs Postgres roles).
...
- **Single onboarding path:** [README, Quick start](https://github.com/neondatabase/neon-for-agent-platforms/blob/main/README.md#quick-start).
